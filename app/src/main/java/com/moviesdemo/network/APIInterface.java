...
public interface APIInterface {
    ...
    @GET("movie/{movie_id}")
    Call<MovieDetails> getMovieDetails(@Path(value = "movie_id", encoded = true) String movie_id, @Query("api_key") String api_key,
                                       @Query("language") String language);

    //https://api.themoviedb.org/3/search/movie?api_key=ec522667fd3810cf76891f7c7ba0782c&language=en-US&query=ven&page=1&include_adult=false
    @GET("search/movie")
    Call<MovieList> searchMovie( @Query("api_key") String api_key, @Query("language") String language, @Query("query") String query,
                                    @Query("page") int page , @Query("include_adult") boolean include_adult);
